  ...
  while (arguments.count > 0) {
    ...
    if (matchingNamedOption) {
      if (matchingNamedOption[kActionOptionSetFlagSelector]) {
          ...
          typedef void (*send_type)(void*, SEL, BOOL);
          
          SEL sel = sel_registerName([matchingNamedOption[kActionOptionSetFlagSelector] UTF8String]);
          send_type methodInstance =(send_type)[[self class] instanceMethodForSelector:sel];
          methodInstance((__bridge void *)(self),sel,YES);
        ...
        if(nextArgument) {
          ...
          *errorMessage = [NSString stringWithFormat:@"The -%@ option requires a parameter.", matchingNamedOption[kActionOptionName]];
          ...
          send_type methodInstance =(send_type)[[self class] instanceMethodForSelector:sel];
          methodInstance((__bridge void *)(self),sel,(__bridge void *)(nextArgument));
    ...
    //  objc_msgSend(self, sel, argument);
        typedef void (*send_type)(void*, SEL, void*);
        send_type methodInstance =(send_type)[[self class] instanceMethodForSelector:sel];
        methodInstance((__bridge void *)(self),sel,(__bridge void *)(argument));
      count++;
